...
#
# Script:
# Description: This script runs configure, builds, and installs gettext
# Distribution: gettext-0.19.8.1
# 
...
# Build gettext package.
# DON'T FORGET TO CD INTO GETTEXT-TOOLS!!!!

{
	time {	EMACS="no" ../configure	--prefix=/tools          \
			--disable-shared > config.out 2>&1 \
		&& make -C gnulib-lib > make.out 2>&1 \
		&& make -C intl pluralx.c >> make.out 2>&1 \
		&& make -C src msgfmt >> make.out 2>&1 \
		&& make -C src msgmerge >> make.out 2>&1 \
		&& make -C src xgettext >> make.out 2>&1 \
		&& cp -v src/{msgfmt,msgmerge,xgettext} /tools/bin > install.out 2>&1 ; \
	}
} 2> $TIMETMPFILE
