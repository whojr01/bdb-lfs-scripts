...
# NOTE: Perl must be built in the distribution directory. e.g. Don't create
# build dir.
...
{
	time { \
		sh Configure -des -Dprefix=/tools -Dlibs=-lm  > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& cp -v perl cpan/podlators/scripts/pod2man /tools/bin > install.out 2>&1 \
		&& mkdir -pv /tools/lib/perl5/5.26.1 >> install.out 2>&1 \
		&& cp -Rv lib/* /tools/lib/perl5/5.26.1 >> install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
