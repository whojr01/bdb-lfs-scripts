...
#
# Script:
# Description: This script runs configure, builds, and installs expect
# Distribution: expect-5.45.3
# 
...
# Build Expect package for scripted program interaction
{
	time { \
		../configure           \
			--prefix=/tools  \
			--with-tclinclude=/tools/include \
			--with-tcl=/tools/lib > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make test > test.out 2>&1 \
		&& make SCRIPTS="" install > install.out 2>&1 ; \
	}
} 2> $TIMETMPFILE
