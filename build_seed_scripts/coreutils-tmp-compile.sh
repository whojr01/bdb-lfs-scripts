...
#
# Script:
# Description: This script runs configure, builds, and installs coreutils
# Distribution: coreutils-8.29
# 
...

{
	time {	../configure	--prefix=/tools          \
			--enable-install-program=hostname > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make RUN_EXPENSIVE_TESTS=yes check > test.out 2>&1 \
