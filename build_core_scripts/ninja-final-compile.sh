...
#
# Script:
# Description: This script runs configure, builds, and installs ninja
# Distribution: ninja-1.8.2
# 
...
{
	time { \
		python3 configure.py --bootstrap > config.out 2>&1 \
		&& python3 configure.py > make.out 2>&1 \
		&& ./ninja ninja_test > test.out 2>&1 \
		&& ./ninja_test --gtest_filter=-SubprocessTest.SetWithLots >> test.out 2>&1 \
		&& install -vm755 ninja /usr/bin > install.out 2>&1 \
		&& install -vDm644 misc/ninja.vim /usr/share/vim/vim80/syntax/ninja.vim >> install.out 2>&1 \
		&& install -vDm644 misc/bash-completion /usr/share/bash-completion/completions/ninja >> install.out 2>&1 \
		&& install -vDm644 misc/zsh-completion /usr/share/zsh/site-functions/_ninja >> install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
