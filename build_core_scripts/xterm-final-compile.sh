...
#
# Script:
# Description: This script runs configure, builds, and installs xterm
# Distribution: xterm-331
# 
...

{
	time { \
		...
		./configure $XORG_CONFIG	\
			--with-app-defaults=/etc/X11/app-defaults > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make install > install.out 2>&1 \
		&& make install-ti >> install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
