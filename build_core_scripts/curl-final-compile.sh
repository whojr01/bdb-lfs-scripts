...
#
# Script:
# Description: This script runs configure, builds, and installs curl
# Distribution: curl-7.55.1
# 
...

{
	time { \
		../configure --prefix=/usr	\
			...
			--with-ca-path=/etc/ssl/certs > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make check > test.out 2>&1 \
		&& make install > install.out 2>&1  \
		&& rm -rfv docs/examples/.deps	>> install.out 2>&1 \
		&& find docs \( -name Makefile\* \
				-o -name \*.1	\
				-o -name \*.3 \) \
				-exec rm -v {} \;	>> install.out 2>&1 \
		&& install -v -d -m755 /usr/share/doc/curl-7.55.1 >> install.out 2>&1 \
		&& cp -v -R docs/* /usr/share/doc/curl-7.55.1 >> install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
