...
#
# Script:
# Description: This script runs configure, builds, and installs grub
# Distribution: grub-2.02
# 
...

{
	time { \
		../configure --prefix=/usr	\
			...
			--target=x86_64		\
			--with-platform=efi	\
			--program-transform-name=s,grub,grub2, > config.out 2>&1  \
		&& make > make.out 2>&1 \
		&& make unicode.pf2 euro.pf2 ascii.pf2 >> make.out 2>&1 \
