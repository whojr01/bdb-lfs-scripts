...
# ***********************************************************
# NOTE: THIS MUST BE BUILT AFTER THE ncurses-final-compile.sh
# ***********************************************************
...
#
# Script:
# Description: This script runs configure, builds, and installs ncurses-5
# Distribution: ncurses-6.0
# 
...

{
	time { \
		../configure --prefix=/usr	\
			...
			--without-debug		\
			--without-cxx-binding	\
			--with-abi-version=5 > config.out 2>&1 \
		&& make sources libs > make.out 2>&1 \
		&& cp -av lib/lib*.so.5* /usr/lib > install.out;
	}
} 2> $TIMETMPFILE
