...
# Instructions for this are from BLFS
# URL: http://www.linuxfromscratch.org/blfs/view/svn/postlfs/cracklib.html
#
# NOTE: Added cracklib from url:
...
#
# Script:
# Description: This script runs configure, builds, and installs cracklib
# Distribution: cracklib-2.9.6
# 
...
{
	time { 
		cp -v ../../cracklib-words-2.9.6.gz .. &&
		sed -i '/skipping/d' ../util/packer.c &&
		../configure --prefix=/usr    \
			--disable-static \
			--with-default-dict=/lib/cracklib/pw_dict > config.out 2>&1 &&
		make > make.out 2>&1 &&
		make install > install.out 2>&1 &&
		mv -v /usr/lib/libcrack.so.* /lib >> install.out 2>&1 &&
		ln -sfv ../../lib/$(readlink /usr/lib/libcrack.so) /usr/lib/libcrack.so >> install.out 2>&1 &&
		install -v -m644 -D ../cracklib-words-2.9.6.gz \
			 /usr/share/dict/cracklib-words.gz >> install.out 2>&1 &&
		gunzip -v /usr/share/dict/cracklib-words.gz >> install.out 2>&1 &&
		ln -v -sf cracklib-words /usr/share/dict/words >> install.out 2>&1 &&
		echo $(hostname) >> /usr/share/dict/cracklib-extra-words &&
		install -v -m755 -d /lib/cracklib >> install.out 2>&1 &&
		create-cracklib-dict /usr/share/dict/cracklib-words	\
			 /usr/share/dict/cracklib-extra-words >> install.out 2>&1
	}
