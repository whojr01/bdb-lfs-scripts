...
#
# Script:
# Description: This script runs configure, builds, and installs jsoncpp
# Distribution: jsoncpp-1.8.4
# 
...

{
	time { \
		BUILD_TYPE=release
		LIB_TYPE=shared
		meson --buildtype ${BUILD_TYPE} --prefix=/usr --default-library ${LIB_TYPE} . build-${LIB_TYPE} > make.out 2>&1 \
		&& ninja -v -C build-${LIB_TYPE} test > test.out 2>&1 \
		&& cd build-${LIB_TYPE} \
		&& DESTDIR= ninja install > ../install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
