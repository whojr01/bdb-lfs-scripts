...
#
# Script:
# Description: This script runs configure, builds, and installs libcap
# Distribution: libcap-2.25
# 
...

{
	time { \
		make > make.out 2>&1 \
		&& make RAISE_SETFCAP=no lib=lib prefix=/usr install > install.out 2>&1 \
