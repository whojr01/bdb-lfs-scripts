...
#
# Script:
# Description: This script runs configure, builds, and installs glib
# Distribution: glib-2.26 (with patches applied)
# 
...
# Configure and make the final glibc

PARAMFLAGS="gcc -isystem /usr/lib/gcc/x86_64-pc-linux-gnu/7.2.0/include -isystem /usr/include"

{
	time {
		CC=$PARAMFLAGS ../configure	\
			--prefix=/usr	\
			--disable-werror	\
			--enable-kernel=3.2	\
			--enable-stack-protector=strong	\
			libc_cv_slibdir=/lib > config.out 2>&1 && \
			make > make.out 2>&1 && \
			make check > test.out 2>&1 && \
			echo Touching /etc/ld.so.conf > install.out && \
			touch /etc/ld.so.conf && \
			echo Disabling PERL script for test-installation >> install.out && \
			sed '/test-installtion/s@$(PERL)@echo not running@' -i ../Makefile && \
