...
#
# Script:
# Description: This script runs configure, builds, and installs cpio
# Distribution: cpio-2.12
# 
...

{
	time { \
		../configure --prefix=/usr	\
			--bindir=/bin		\
			--enable-mt		\
			--with-rmt=/usr/libexec/rmt > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& makeinfo --html -o doc/html ../doc/cpio.texi >> make.out 2>&1 	\
		&& makeinfo --html --no-split -o doc/cpio.html ../doc/cpio.texi >> make.out 2>&1 \
		&& makeinfo --plaintext -o doc/cpio.txt ../doc/cpio.texi >> make.out 2>&1 \
		&& make check > test.out 2>&1 \
		&& make install > install.out 2>&1 \
		&& install -v -m755 -d /usr/share/doc/cpio-2.12/html >> install.out 2>&1 \
		&& install -v -m644 doc/html/* /usr/share/doc/cpio-2.12.html  >> install.out 2>&1 \
		&& install -v -m644 doc/cpio.{html,txt} /usr/share/doc/cpio-2.12 >> install.out 2>&1;
	}
} 2> $TIMETMPFILE
