...
#
# Script:
# Description: This script runs configure, builds, and installs dejavu-fonts-tff
# Distribution: dejavu-fonts-tff-2.37
# 

BUILD=dejavu-fonts-tff
STAGE=2
PASS=1
SCRIPT=$0
...
{
	time { \
		install -v -d -m755 /usr/share/fonts/dejavu > install.out 2>&1 &&
		install -v -m644 ttf/*.ttf /usr/share/fonts/dejavu >> install.out 2>&1 &&
		fc-cache -v /usr/share/fonts/dejavu >> install.out 2>&1; 
	}
} 2> $TIMETMPFILE
