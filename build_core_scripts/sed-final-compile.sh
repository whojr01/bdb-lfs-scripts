...
#
# Script:
# Description: This script runs configure, builds, and installs sed
# Distribution: sed-4.4
# 
...

{
	time { \
		../configure --prefix=/usr	\
			      --bindir=/bin > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make html >> make.out 2>&1 \
		&& make check > test.out 2>&1 \
		&& make install > install.out 2>&1 \
		&& install -d -m755 /usr/share/doc/sed-4.4 >> install.out 2>&1 \
		&& install -m644 doc/sed.html /usr/share/doc/sed-4.4 >> install.out 2>&1 ;
	}
