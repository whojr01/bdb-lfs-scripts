...
#
# Script:
# Description: This script runs configure, builds, and installs python
# Distribution: Python-3.6.4
# 
...

{
	time { \
		../configure --prefix=/usr	\
			...
			--with-systerm-expat	\
			--with-system-ffi	\
			--with-ensurepip=yes > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make install > install.out 2>&1 \
		&& chmod -v 755 /usr/lib/libpython3.6m.so >> install.out 2>&1 \
		&& chmod -v 755 /usr/lib/libpython3.so >> install.out 2>&1 \
		&& install -v -dm755 /usr/share/doc/python-3.6.4/html >> install.out 2>&1 \
		&& tar --strip-components=1 \
			--no-same-owner \
			--no-same-permissions \
			-C /usr/share/doc/python-3.6.4/html \
			-xvf ../../python-3.6.4-docs-html.tar.bz2 >> install.out 2>&1;
	}
} 2> $TIMETMPFILE
