...
#
# Script:
# Description: This script runs configure, builds, and installs dbus-python
# Distribution: dbus-python-1.2.6
# 
...

{
	time { \
		mkdir python2 	\
		&& pushd python2 > /dev/null	\
		&& PYTHON=/usr/bin/python	\
			../configure --prefix=/usr \
				--docdir=/usr/share/doc/dbus-python-1.2.4 > ../config.out 2>&1 \
		&& make > ../make.out 2>&1 \
		...
		mkdir python3	\
		&& pushd python3 > /dev/null	\
		&& PYTHON=/usr/bin/python3	\
		&& ../configure --prefix=/usr --docdir=/usr/share/doc/dbus-python-1.2.4 >> ../config.out 2>&1 \
		&& make >> ../make.out 2>&1 \
		&& make check >> ../test.out 2>&1 \
		&& make install >> ../install.out 2>&1 ;
	}
