...
#
# Script:
# Description: This script runs configure, builds, and installs util-linux
# Distribution:  (E.g. whatever-1.23.4)
# 
...

{
	time { \
		../configure ADJTIME_PATH=/var/lib/hwclock/adjtime	\
			...
			--disable-pylibmount	\
			--disable-static	\
			--without-python > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& chown -Rv nobody . > test.out 2>&1 \
		&& su nobody -s /bin/bash -c "PATH=$PATH make RUN_EXPENSIVE_TESTS=yes check" >> test.out 2>&1 \
