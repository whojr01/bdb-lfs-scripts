...
#
# Script:
# Description: This script runs configure, builds, and installs sgml-common
# Distribution: sgml-common-0.6.3
# 
...

{
	time { \
		./configure --prefix=/usr	\
			--sysconfdir=/etc > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make docdir=/usr/share/doc install > install.out 2>&1 \
		&& install-catalog --add /etc/sgml/sgml-ent.cat \
			/usr/share/sgml/sgml-iso-entities-8879.1986/catalog >> install.out 2>&1 \
		&& install-catalog --add /etc/sgml/sgml-docbook.cat \
			/etc/sgml/sgml-ent.cat >> install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
