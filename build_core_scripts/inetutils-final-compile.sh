...
#
# Script:
# Description: This script runs configure, builds, and installs inetutils
# Distribution: inetutils-1.9.4
# 
...

{
	time { \
		../configure --prefix=/usr	\
				...
				--disable-talk \
				--disable-telnet \
			--disable-servers > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make check > test.out 2>&1 \
		&& make install > install.out 2>&1 \
		&& mv -v /usr/bin/{hostname,ping,ping6,traceroute} /bin >> install.out 2>&1 \
		&& mv -v /usr/bin/ifconfig /sbin >> install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
