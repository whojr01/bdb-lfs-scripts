...
#
# Script:
# Description: This script runs configure, builds, and installs libuv
# Distribution: libuv-1.19.1
# 
...
# Install python library gyp into build/gyp before you run build.
# git clone https://chromium.googlesource.com/external/gyp.git build/gyp
#

{
	time { \
		./gyp_uv.py -Duv_library=shared_library > config.out 2>&1 \
		&& BUILDTYPE=Release make -C out > make.out 2>&1 \
		&& ./out/Release/run-tests > test.out 2>&1 ;
	}
} 2> $TIMETMPFILE
