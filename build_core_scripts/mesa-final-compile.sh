...
#
# Script:
# Description: This script runs configure, builds, and installs mesa
# Distribution: mesa-17.3.6
# 
...

{
	time { \
		./autogen.sh CFLAGS='-O2' CXXFLAGS='-O2'	\
			...
			--with-gallium-drivers="i915,r600,nouveau,radeonsi,svga,swrast" > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& echo "Building Demo programs" >> make.out \
		&& make -C xdemos DEMOS_PREFIX=$XORG_PREFIX >> make.out 2>&1 \
		&& make check > test.out 2>&1 \
		&& make install > install.out 2>&1 \
		&& make -C xdemos DEMOS_PREFIX=$XORG_PREFIX install >> install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
