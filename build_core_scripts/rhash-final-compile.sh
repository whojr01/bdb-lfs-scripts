...
#
# Script:
# Description: This script runs configure, builds, and installs rhash
# Distribution: rhash-1.3.5
# 
...
{
	time { \
		make ADDCFLAGS="-DUSE_GETTEXT -DOPENSSL_RUNTIME -rdynamic" \
			ADDLDFLAGS="-ldl" PREFIX=/usr lib-shared > make.out 2>&1 \
		&& LD_LIBRARY_PATH="$(pwd)/librhash" make test > test.out 2>&1 \
		&& make ADDCFLAGS="-DUSE_GETTEXT -DOPENSSL_RUNTIME -rdynamic" \
                        ADDLDFLAGS="-ldl" PREFIX=/usr install-shared > install.out 2>&1 \
		&& make ADDCFLAGS="-DUSE_GETTEXT -DOPENSSL_RUNTIME -rdynamic" \
                        ADDLDFLAGS="-ldl" PREFIX=/usr install > install.out 2>&1 \
		&& make PREFIX=/usr install-lib-static >> install.out 2<&1 \
		&& make PREFIX=/usr install-lib-shared >> install.out 2<&1 ;
	echo "**********************************************************"
	echo "You must manually install the headers"
	echo "cd librhash"
	echo "install -d /usr/include"
	echo "install -m 644 rhash.h rhash_torrent.h /usr/include/"
	echo "***********************************************************"
	}
} 2> $TIMETMPFILE
