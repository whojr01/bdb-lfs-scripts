...
#
# Script:
# Description: This script runs configure, builds, and installs python2
# Distribution: Python-2.7.14
# 
...

{
	time { \
		../configure --prefix=/usr	\
			...
			--with-system-ffi	\
			--with-ensurepip=yes	\
			--enable-unicode=ucs4 > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make -k test > test.out 2>&1	\
		&& make install > install.out 2>&1 \
		&& chmod -v 755 /usr/lib/libpython2.7.so.1.0 >> install.out 2>&1 \
		&& install -v -dm755 /usr/share/doc/python-2.7.14 >> install.out 2>&1 \
		&& tar --strip-components=1 \
			--no-same-owner \
			--no-same-permissions \
			--directory /usr/share/doc/python-2.7.14 \
			-xvf ../../python-2.7.14-docs-html.tar.bz2 >> install.out 2>&1;
	}
} 2> $TIMETMPFILE
