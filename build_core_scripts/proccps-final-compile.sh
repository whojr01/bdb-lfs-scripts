...
#
# Script:
# Description: This script runs configure, builds, and installs procps-ng
# Distribution: procps-ng-3.3.12
# 
...

{
	time { \
		../configure --prefix=/usr	\
			...
			--disable-static	\
			--disable-kill		\
			--with-systemd > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& sed -i.orig -r 's|(pmap_initname)\\\\$|\1|' ../testsuite/pmap.test/pmap.exp > test.out 2>&1 \
		&& sed -i.orig '/set tty/d' ../testsuite/pkill.test/pkill.exp >> test.out 2>&1 \
		&& rm -v ../testsuite/pgrep.test/pgrep.exp \
		&& make check >> test.out 2>&1 \
		&& make install > install.out 2>&1 \
		&& mv -v /usr/lib/libprocps.so.* /lib >> install.out 2>&1 \
		&& ln -sfv ../../lib/$(readlink /usr/lib/libprocps.so) /usr/lib/libprocps.so >> install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
