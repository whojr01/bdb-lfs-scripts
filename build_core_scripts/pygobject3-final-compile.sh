...
#
# Script:
# Description: This script runs configure, builds, and installs pygobject3
# Distribution: pygobject3-3.24.1
# 
	...
	time { \
		...
		&& PYTHON=/usr/bin/python	\
			../configure --prefix=/usr \
				--with-python=/usr/bin/python > ../config.out 2>&1 \
		&& make > ../make.out 2>&1 \
		...
		&& ../configure --prefix=/usr --with-python=/usr/bin/python3 >> ../config.out 2>&1 \
		&& make >> ../make.out 2>&1 \
		&& make check >> ../test.out 2>&1 \
		&& make install >> ../install.out 2>&1 ;
	}
