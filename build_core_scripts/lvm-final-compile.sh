...
#
# Script:
# Description: This script runs configure, builds, and installs lvm2
# Distribution: lvm2-2.02.177
# 
...

{
	time { \
		./configure --prefix=/usr	\
			...
			--enable-cmdlib		\
			--enable-pkgconfig	\
			--enable-udev_sync	> config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make -C tools install_dmsetup_dynamic > install.out 2>&1 \
		&& make -C udev install >> install.out 2>&1 \
		&& make -C libdm install >> install.out 2>&1 \
		&& make check_local > test.out 2>&1 \
