...
#
# Script:
# Description: This script runs configure, builds, and installs xfsprogs
# Distribution: xfsprogs-4.12.0
# 
...

{
	time { \
		make DEBUG=-DNDEBUG     \
			INSTALL_USER=root  \
			INSTALL_GROUP=root \
			LOCAL_CONFIGURE_OPTIONS="--enable-readline" > make.out 2>&1
		make PKG_DOC_DIR=/usr/share/doc/xfsprogs-4.12.0 install > install.out 2>&1    &&
		make PKG_DOC_DIR=/usr/share/doc/xfsprogs-4.12.0 install-dev >> install.out 2>&1  &&

		rm -rfv /usr/lib/libhandle.a >> install.out 2>&1       &&
		rm -rfv /lib/libhandle.{a,la,so} >> install.out 2>&1   &&
		ln -sfv ../../lib/libhandle.so.1 /usr/lib/libhandle.so >> install.out 2>&1 &&
		sed -i "s@libdir='/lib@libdir='/usr/lib@" /usr/lib/libhandle.la >> install.out 2>&1 
	}
} 2> $TIMETMPFILE
