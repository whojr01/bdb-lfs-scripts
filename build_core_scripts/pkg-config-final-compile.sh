...
#
# Script:
# Description: This script runs configure, builds, and installs pkg-config
# Distribution: pkg-config-0.29.2
# 
...

{
	time { \
		../configure --prefix=/usr	\
			--with-internal-glib \
			--disable-host-tool \
			--docdir=/usr/share/doc/pkg-config-0.29.2 > config.out 2>&1 \
		&& make > make.out 2>&1 \
