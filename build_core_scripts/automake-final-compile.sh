...
#
# Script:
# Description: This script runs configure, builds, and installs automake
# Distribution: automake-1.15.1
# 
...

{
	time { \
		../configure --prefix=/usr	\
			--docdir=/usr/share/doc/automake-1.15.1 > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make -j4 check > test.out 2>&1 \
