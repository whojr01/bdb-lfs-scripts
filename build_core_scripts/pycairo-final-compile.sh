...
#
# Script:
# Description: This script runs configure, builds, and installs pycairo
# Distribution: pycairo-1.15.1
# 
...
{
	time { \
		python2 setup.py build > make.out 2>&1	\
		&& python2 setup.py install --optimize=1 > install.out 2>&1 \
		&& rm -rf build

		python3 setup.py build >> make.out 2>&1 	\
		&& python3 setup.py install --optimize=1 >> install.out 2>&1 
	}
} 2> $TIMETMPFILE
