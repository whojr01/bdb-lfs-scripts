...
#
# Script:
# Description: This script runs configure, builds, and installs gmp
# Distribution:  gmp-6.1.2
# 
...
# This script builds, tests, and installs GMP.

{
	time {
		../configure --prefix=/usr	\
			--enable-cxx	\
			--disable-static	\
			--docdir=/usr/share/doc/gmp-6.1.2 > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make html >> make.out 2>&1 \
		&& make check 2>&1 | tee test.out \
		&& make install > install.out 2>&1 \
		&& make install-html >> install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
