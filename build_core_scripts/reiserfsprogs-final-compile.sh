...
#
# Script:
# Description: This script runs configure, builds, and installs reiserfsprogs
# Distribution: reiserfsprogs-3.6.27
# 
...
{
	time { \
		autoreconf -fiv > config.out 2>&1 \
		&& ./configure --prefix=/usr	\
			--sbindir=/sbin >> config.out 2>&1 \
		&& make > make.out 2>&1  \
