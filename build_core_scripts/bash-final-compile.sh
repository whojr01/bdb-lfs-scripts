...
#
# Script:
# Description: This script runs configure, builds, and installs bash
# Distribution: bash-4.4.12
# 
...

{
	time { \
		../configure --prefix=/usr	\
			...
			--without-bash-malloc \
			--with-installed-readline > config.out 2>&1 && \
		make > make.out 2>&1 && \
		chown -Rv nobody . && \
		su nobody -s /bin/bash -c "PATH=$PATH make tests" |& tee test.out && \
		chown -Rv root:root . && \
