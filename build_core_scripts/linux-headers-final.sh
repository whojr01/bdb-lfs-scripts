...
#
# Script:
# Description: This script runs configure, builds, and installs linux-hdrs
# Distribution: linux-4.14.13
# 
...
# This installs the linux kernel headers

{
	time {
		make mrproper > make.out 2>&1
		make INSTALL_HDR_PATH=dest headers_install > install.out 2>&1
		find dest/include \( -name .install -o -name ..install.cmd \) -delete >> install.out 2>&1
		cp -rv dest/include/* /usr/include >> install.out 2>&1
	}
