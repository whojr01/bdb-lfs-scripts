...
#
# Script:
# Description: This script runs configure, builds, and installs qemu
# Distribution: qemu-2.11.0
# 
...

{
	time { \
		../configure --prefix=/usr	\
			...
			--audio-drv-list=alsa	\
			--with-sdlabi=2.0	\
			--docdir=/usr/share/doc/qemu-2.11.1 > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make V=1 -k check > test.out 2>&1 \
