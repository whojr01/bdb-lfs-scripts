...
#
# Script:
# Description: This script runs configure, builds, and installs openssl
# Distribution: openssl-1.1.0g
# 
...

{
	time { \
		../config --prefix=/usr	\
			...
			--libdir=lib		\
			shared			\
			zlib-dynamic > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make test > test.out 2>&1 \
		&& make MANSUFFIX=ssl install > install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
