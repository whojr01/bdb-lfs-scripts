...
#
# Script:
# Description: This script runs configure, builds, and installs xorg_proto_hdrs
# Distribution: xorg_proto_hdrs
# 

BUILD=xorg_proto_header
STAGE=2
PASS=1
SCRIPT=$0
TIMETMPFILE=/tmp/$$-tmp-build.dat

if [ ! -f ../proto-7.md5 ]; then
	echo "Environment is not properly configured. See:"
	echo "http://www.linuxfromscratch.org/blfs/view/svn/x/x7proto.html"
	exit 1
fi

{
	time { \
		...
		do
			echo "###############################" >> config.out
			echo "# $package #" >> config.out
			echo "###############################" >> config.out
			packagedir=${package%.tar.bz2}
			tar -xvf $package >> config.out 2>&1
			if [ $? -ne 0 ]; then echo "Tar Failure: $package"; exit 1; fi
			pushd $packagedir
				./configure $XORG_CONFIG >> ../config.out 2>&1
				if [ $? -ne 0 ]; then echo "Configure Failure: $package" >> ../config.out ; exit 1; fi
				echo "###############################" >> ../install.out
				echo "# $package #" >> ../install.out
				echo "###############################" >> ../install.out
				make install >> ../install.out 2>&1
				if [ $? -ne 0 ]; then echo "Install Failure: $package" >> install.out ; exit 1; fi
			popd
			rm -rf $packagedir
		done
	}
