...
#
# Script:
# Description: This script runs configure, builds, and installs readline
# Distribution: readline-7.0
# 
...
#
# This script will build and install readline since there is
# no test suite.
#

{
	time { \
		../configure --prefix=/usr \
			--disable-static \
			--docdir=/usr/share/doc/readline-7.0 > config.out 2>&1 \
		&& make SHLIB_LIBS="-L/tools/lib -lncursesw" > make.out 2>&1 \
		&& make SHLIB_LIBS="-L/tools/lib -lncursesw" install > install.out 2>&1 \
		&& mv -vf /usr/lib/lib{readline,history}.so.* /lib >> install.out 2>&1 \
		&& ln -svf ../../../lib/$(readlink /usr/lib/libreadline.so) /usr/lib/libreadline.so >> install.out 2>&1 \
		&& ln -svf ../../../lib/$(readlink /usr/lib/libhistory.so) /usr/lib/libhistory.so >> install.out 2>&1 \
		&& install -v -m644 ../doc/*.{ps,pdf,html,dvi} /usr/share/doc/readline-7.0 >> install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
