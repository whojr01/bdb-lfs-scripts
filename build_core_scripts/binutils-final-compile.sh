...
#
# Script:
# Description: This script runs configure, builds, and installs binutils
# Distribution: binutils-2.29.1
# 
...
# This script builds the temporary binutils package. It is also used as
# the SBU (System Build Unit) for measuring time.
#
#

{
	time {
		../configure --prefix=/usr \
			...
			--enable-shared \
			--disable-werror \
			--with-system-zlib > config.out 2>&1 \
		&& make tooldir=/usr > make.out 2>&1 \
		&& make -k check > test.out 2>&1 \
		&& make tooldir=/usr install > install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
