...
#
# Script:
# Description: This script runs configure, builds, and installs pciutils
# Distribution: pciutils-3.5.5
# 
...

{
	time { \
		make PREFIX=/usr	\
			...
			SHARED=yes > make.out 2>&1	\
		&& make PREFIX=/usr	\
			SHAREDDIR=/usr/share/hwdata	\
			SHARED=yes	\
			install install-lib > install.out 2>&1	\
		&& chmod -v 755 /usr/lib/libpci.so >> install.out 2>&1
	}
} 2> $TIMETMPFILE
