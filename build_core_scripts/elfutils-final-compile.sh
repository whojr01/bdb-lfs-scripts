...
#
# Script:
# Description: This script runs configure, builds, and installs elfutils
# Distribution: elfutils-0.170
# 
...
{
	time { \
		../configure --prefix=/usr  > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make check > test.out 2>&1 \
		&& make -C libelf install > install.out 2>&1 \
