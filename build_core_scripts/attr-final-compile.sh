...
#
# Script:
# Description: This script runs configure, builds, and installs attr
# Distribution: attr-2.4.47
# 
...

{
	time { \
		../configure --prefix=/usr	\
			--disable-static > config.out 2>&1 \
		&& make > make.out 2>&1 \
		&& make -j1 tests root-tests > test.out 2>&1 \
		&& make install install-dev install-lib > install.out 2>&1 ;
	}
} 2> $TIMETMPFILE
