...
#
# Script:
# Description: This script runs configure, builds, and installs sqlite
# Distribution: sqlite-autoconf-3220000
# 
...

{
	time { \
		./configure --prefix=/usr	\
			...
			--enable-fts5		\
			CFLAGS="-g -O2		\
			-DSQLITE_ENABLE_FST4=1	\
			-DSQLITE_ENABLE_COLUMN_METADATA=1	\
			-DSQLITE_ENABLE_UNLOCK_NOTIFY=1		\
			-DSQLITE_ENABLE_DBSTAT_VTAB=1		\
			-DSQLITE_SECURE_DELETE=1		\
			-DSQLITE_ENABLE_FTS3_TOKENIZER=1" > config.out 2>&1 \
		&& make > make.out 2>&1 \
