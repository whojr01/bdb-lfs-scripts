...
#
# Script:
# Description: This script runs configure, builds, and installs boost
# Distribution: boost_1_66_0
# 
...
{
	time { \
		./bootstrap.sh --prefix=/usr > make.out 2>&1 &&
		./b2 stage threading=multi link=shared >> make.out 2>&1
		pushd tools/build/test > test.out 2>&1
			mv -v zlib.py zlib.py-orig >> test.out 2>&1
			python test_all.py >> test.out 2>&1
		popd >> test.out 2>&1
		cat tools/build/test/test.out >> test.out 2>&1
		./b2 install threading=multi link=shared > install.out 2>&1
	}
} 2> $TIMETMPFILE
